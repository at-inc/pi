...
```bash
npm install @at-inc/pi-ai
```

TypeBox exports are re-exported from `@at-inc/pi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@at-inc/pi-ai';
import { builtinModels } from '@at-inc/pi-ai/providers/all';
...
import { openrouterProvider } from '@at-inc/pi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@at-inc/pi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@at-inc/pi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@at-inc/pi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@at-inc/pi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@at-inc/pi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@at-inc/pi-ai';
...
```typescript
import { builtinImagesModels } from '@at-inc/pi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@at-inc/pi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@at-inc/pi-ai';
import { openAICompletionsApi } from '@at-inc/pi-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@at-inc/pi-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@at-inc/pi-ai/api/openai-responses.lazy';
...
```typescript
import { normalizeContext } from '@at-inc/pi-ai';
import { stream } from '@at-inc/pi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@at-inc/pi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@at-inc/pi-ai';
import { anthropicProvider } from '@at-inc/pi-ai/providers/anthropic';
import { openaiProvider } from '@at-inc/pi-ai/providers/openai';
import { googleProvider } from '@at-inc/pi-ai/providers/google';
...
```typescript
import { getCurrentSystemPrompt, getCurrentTools } from "@at-inc/pi-ai";
...
- `@at-inc/pi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@at-inc/pi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@at-inc/pi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@at-inc/pi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@at-inc/pi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@at-inc/pi-ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @at-inc/pi-ai login              # interactive provider selection
npx @at-inc/pi-ai login anthropic    # login to specific provider
npx @at-inc/pi-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@at-inc/pi-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@at-inc/pi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@at-inc/pi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@at-inc/pi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@at-inc/pi-ai`.
