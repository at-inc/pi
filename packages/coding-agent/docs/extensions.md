...
| Package | Purpose |
|---------|---------|
| `@at-inc/pi` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@at-inc/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
...
```typescript
import { isToolCallEventType } from "@at-inc/pi";
...
```typescript
import { isBashToolResult } from "@at-inc/pi";
...
```typescript
import { createLocalBashOperations } from "@at-inc/pi";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@at-inc/pi";
...
```typescript
import { SessionManager } from "@at-inc/pi";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@at-inc/pi-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@at-inc/pi-ai";
...
```typescript
import { withFileMutationQueue } from "@at-inc/pi";
...
**Important:** Use `StringEnum` from `@at-inc/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@at-inc/pi";
...
```typescript
import { createBashTool } from "@at-inc/pi";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@at-inc/pi";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@at-inc/pi";
