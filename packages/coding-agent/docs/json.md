...
Use the exported `JsonAgentSessionEvent` type from `@at-inc/pi`. Its implementation is in [`json-event.ts`](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/src/modes/json-event.ts).
