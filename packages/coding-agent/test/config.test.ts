...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@at-inc/pi")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @at-inc/pi",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@at-inc/pi")).toBeUndefined();
		expect(getUpdateInstruction("@at-inc/pi")).toBe(
			"Update @at-inc/pi using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@at-inc/pi");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@at-inc/pi"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @at-inc/pi`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@at-inc/pi", undefined, {
			packageName: "@at-inc/pi",
			installSpec: "@at-inc/pi@1.2.3",
		});

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@at-inc/pi@1.2.3"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @at-inc/pi@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@at-inc/pi", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @at-inc/pi`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@at-inc/pi")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @at-inc/pi",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@at-inc/pi"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @at-inc/pi",
		});
	});
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@at-inc/pi")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@at-inc/pi")).toContain("the install path is not writable");
	});
});
