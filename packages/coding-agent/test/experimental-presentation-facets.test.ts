...
describe("server-selected presentation facets", () => {
	...
	test("builds conventional plugin entries into the server-owned plugin cache", async () => {
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@earendil-works/chord"; import { SlashCommands } from "@at-inc/pi/experimental/plugin"; export default defineFacet({ id: "built-a", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@earendil-works/chord"; import { SlashCommands } from "@at-inc/pi/experimental/plugin"; export default defineFacet({ id: "built-b", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@earendil-works/chord"; import { SlashCommands } from "@at-inc/pi/experimental/plugin"; export default defineFacet({ id: "built-c", setup(env) { env.use(SlashCommands); } });\n',
		);
